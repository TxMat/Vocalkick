...
async def change_presence(message, *args):
    ...
    await client.get_user(259676097652719616).dm_channel.send(str(log)) 
...
async def addd(message, *args):
    ...
    if nom in classe:
            ...
            if nom not in here:
                ...
                await message.add_reaction("❌")
                await client.get_user(259676097652719616).dm_channel.send(here)
                await client.get_user(259676097652719616).dm_channel.send("deja dans le serv")
...
async def veriff(message, *args):
    ...
    if message.guild.id != 757272034157002772:
        await client.get_user(259676097652719616).dm_channel.send("invalid server for verif")
        await message.add_reaction("❔")
        return
    nom = " ".join(args).lower()
    await client.get_user(259676097652719616).dm_channel.send(args)
    try:
        ...
        if nom in classe:
            ...
            if nom not in here:
                await message.add_reaction("👌")
                await client.get_user(259676097652719616).dm_channel.send("renaming...")
                rename = args[-1][0].upper() + args[-1][1:].lower() + " " + args[0][0].upper() + "."
                await message.author.edit(nick=rename)
                here.append(nom)
                await client.get_user(259676097652719616).dm_channel.send("recherche du groupe...")
                groupe = classe[nom]
                ...
                if "A" in groupe:
                    await client.get_user(259676097652719616).dm_channel.send("groupe A")
                    await message.author.add_roles(get(message.guild.roles, name="A"))
                    await client.get_user(259676097652719616).dm_channel.send("task sucsess")
                    return
                elif "B" in groupe:
                    await client.get_user(259676097652719616).dm_channel.send("groupe B")
                    await message.author.add_roles(get(message.guild.roles, name="B"))
                    await client.get_user(259676097652719616).dm_channel.send("task sucsess")
                    return
                elif "C" in groupe:
                    await client.get_user(259676097652719616).dm_channel.send("groupe C")
                    await message.author.add_roles(get(message.guild.roles, name="C"))
                    await client.get_user(259676097652719616).dm_channel.send("task sucsess")
                    return
                elif "D" in groupe:
                    await client.get_user(259676097652719616).dm_channel.send("groupe D")
        ...
        await message.add_reaction("❔")
        await client.get_user(259676097652719616).dm_channel.send("critical error")
...
async def chatt(message, *args):
    ...
    try:
        ...
        if args[0] == "stop":
            ...
            await message.add_reaction("👌")
            return
        await client.get_user(259676097652719616).dm_channel.send("I had a invalid id can't process to chat")
        await message.add_reaction("❔")
        return
        ...
        await client.get_user(259676097652719616).dm_channel.send("I don't recive any id I don't know what to do")
        await message.add_reaction("❔")
        await message.author.create_dm()
...
async def helpp(message, *args):
    ...
    await message.add_reaction(OPTIONS[message.channel.guild.id]["emoji"])
    await client.get_user(259676097652719616).dm_channel.send("help succsesfully send")
actions = {"option": option,"add": addd,"verif": veriff,"desc": change_presence, "stop": toogle_stop, "help": helpp, "mute": mutee, "chat": chatt, "say": sayy, "unmute": unmutee, "ban": bann}
...
async def on_message(message):
        ...
        for mot in tg:
            if mot in a:                
                if message.author.id == 259676097652719616:
                    await client.get_user(259676097652719616).dm_channel.send("EYES INTENSIFIES")
                    await message.add_reaction("👀")
                    return
    ...
    if len(message.content) and message.content[0] == OPTIONS[message.guild.id]["prefix"]:
        ...
        if a[0] in perm_actions and (message.author.guild_permissions.move_members == False and OPTIONS[message.channel.guild.id]["role"] not in list(map(lambda x: x.name, message.author.roles))):
            ...
            await client.get_user(259676097652719616).dm_channel.send(log)
            await message.add_reaction("❌")
            await client.get_user(259676097652719616).dm_channel.send(message.author.roles)
            return
...
async def on_ready():
    await client.get_user(259676097652719616).create_dm()
    await client.get_user(259676097652719616).dm_channel.send("**RESTART**")
    await client.get_user(259676097652719616).dm_channel.send('{} is online and ready to kick'.format(client.user))
    mute = False
...
async def on_delay(channel, first=False):
    ...
    if not OPTIONS[channel.guild.id]["running"]:
        ...
        return
    if first:
        log = channel.members[0].name, "is alone in", channel.name, "waiting for someone in the server :", channel.guild.name,
        await client.get_user(259676097652719616).dm_channel.send(log)
        log = "starting the alone time countdown before kicking(", OPTIONS[channel.guild.id]["frst_time"], "sec )"
        await client.get_user(259676097652719616).dm_channel.send(log)
        await asyncio.sleep(OPTIONS[channel.guild.id]["frst_time"])
        try:
            log = "Countdown ended for", channel.members[0].name, "in", channel.name, "(", channel.guild.name, ")"
            await client.get_user(259676097652719616).dm_channel.send(log)
        except:
            log = "Countdown ended but the member has left the channel before"
            await client.get_user(259676097652719616).dm_channel.send(log)
    else:
        log = channel.members[0].name, "is now alone in the channel", channel.name, "in the server :", channel.guild.name
        await client.get_user(259676097652719616).dm_channel.send(log)
        log = "starting the alone time countdown before kicking(", OPTIONS[channel.guild.id]["alone_time"], "sec )"
        await client.get_user(259676097652719616).dm_channel.send(log)
    ...
    if channel in CHANNELS:
        ...
        if nb != 1:
            if nb > 1:
                log = "there is now", nb, "person connected in", channel.name, "(", channel.guild.name, ") with", members[0], "aborting kicking procedure..."
                await client.get_user(259676097652719616).dm_channel.send(log)
                return
            if nb == 0:
                ...
                await client.get_user(259676097652719616).dm_channel.send(log)
                return
        log = members[0], "is still alone in", channel.name, "on", channel.guild, "starting kicking procedure..."
        await client.get_user(259676097652719616).dm_channel.send(log)
        ...
        try:
            ...
            if e.code == 50007:
                log = "can't send dm to", members[0], "the user must have bocked me"
                await client.get_user(259676097652719616).dm_channel.send(log)
